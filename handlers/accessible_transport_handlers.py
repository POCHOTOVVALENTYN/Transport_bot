...
async def _show_accessible_transport_results(query, stop_title: str, routes: list, route_titles: list = None):
    ...
    """
    # СЦЕНАРІЙ 1: На зупинці "пусто" (EasyWay не бачить прибуття)
    if not routes:
        ...
        if route_titles:
            ...
            # Перевіряємо кожен маршрут у нашому Моніторі
            for route_num in route_titles:
                # Питаємо: "Де зараз вагони маршруту №5?"
                vehicles_info = monitoring_service.get_accessible_on_route(route_num)

                if vehicles_info:
                    # Якщо знайшли - формуємо гарний список
                    # vehicles_info - це список рядків типу "🚋 4015 (біля: Вокзал)"
                    vh_str = "\n".join([f"   └ {v}" for v in vehicles_info])
                    found_lines.append(f"<b>Маршрут №{route_num}:</b>\n{vh_str}")

            if found_lines:
                has_global_info = True
                monitoring_text = (
                        "\n🔎 <b>Але я знайшов їх в іншому місці міста:</b>\n\n" +
                        "\n".join(found_lines) +
                        "\n\n<i>(Вони рухаються, просто ще далеко від Вас)</i>"
                )

                # Формуємо фінальне повідомлення
            if has_global_info:
            # Варіант: Знайшли в місті
                message = (
                ...
                # Варіант: Не знайшли ніде (або ніч, або аварія)
                message = (
                    f"♿️ <b>На зупинці '{stop_title}'</b>\n"
                    f"───────────────\n"
                    f"😔 <b>На жаль, зараз немає інформації.</b>\n\n"
                    f"Я перевірив все місто, але не знайшов активних низькопідлогових вагонів на маршрутах, "
                    f"що проходять тут.\n\n"
                    f"Можливо:\n"
                    f"• Вагони ще в депо\n"
                    f"• Тимчасова зупинка руху\n"
                    f"• Відсутній зв'язок GPS"
                )

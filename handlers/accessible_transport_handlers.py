...
async def _render_accessible_response(query, stop_title: str, stop_info: dict, global_route_data: dict,
    ...
    for key in sorted_keys:
        ...
        # === СЦЕНАРІЙ Б: НЕМАЄ ПРОГНОЗУ, АЛЕ Є GPS ===
            ...
            if vehicles_count > 0:
                ...
                # ЗМІНЕНО: Замість цифри виводимо загальне повідомлення
                message += f"⚡️ На даному маршруті працює електротранспорт!\n"

                message += (
                    f"ℹ️ <i>Електротранспорт вже проїхав Вашу зупинку або рухається в іншому напрямку ↩️ "
                    f"(якщо на маршруті 1 од.)\n"
    ...
    if not has_data:
        message += (
            "😕 <b>Інформація про низькопідлогові маршрути на цій зупинці наразі відсутня.</b>\n\n"
            "👀 <i>Можливі причини:</i>\n"
            ...
            "🔌 <b>Технічні причини</b> <i>(напр.'відсутність світла')</i>\n\n"
        )

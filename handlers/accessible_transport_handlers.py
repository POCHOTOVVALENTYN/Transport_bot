...
async def _show_accessible_transport_results(query, stop_title: str, routes: list):
    ...
    if not routes:
        ...
        message = (
            ...
            f"───────────────\n\n"
            f"😔 <b>На жаль, поки що немає інформації...</b>\n\n"
            f"❌ На цьому напрямку  <b>ще не зафіксовано</b> низькопідлогового транспорту, "
            f"що прямує до Вас.\n\n"
            f"🔄 <b>Порада:</b> Спробуйте будь ласка оновити запит через декілька хвилин!"
        )
        keyboard = [
